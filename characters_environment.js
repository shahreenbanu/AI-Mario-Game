...
var nose_y = "";
var situation = "";
function MarchOnMario() {
  
  document.getElementById("status").innerHTML="Get Ready !!";
  situation = "start";
}
function game(){
console.log(nose_x + "and ..... " + nose_y);
  instializeInDraw();
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Press the Play With Mario Button to start the game ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if( situation == "start" && gameConfig.status==="start" && nose_x != " ") {
    world_start.play();
